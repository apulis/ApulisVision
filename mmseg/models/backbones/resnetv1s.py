...
class ResNetV1s(nn.Module):
    ...
    arch_settings = {
        18: (BasicBlockV1s, (2, 2, 2, 2)),
        34: (BasicBlockV1s, (3, 4, 6, 3)),
        50: (BottleneckV1s, (3, 4, 6, 3)),
        101: (BottleneckV1s, (3, 4, 23, 3)),
        152: (BottleneckV1s, (3, 8, 36, 3))
    }
    ...
    def _make_stem_layer(self, in_channels, deep_stem=False):
        if deep_stem:
    ...
    def init_weights(self, pretrained=None):
        if isinstance(pretrained, str):
            ...
            if self.dcn is not None:
                for m in self.modules():
                    if isinstance(m, BottleneckV1s) and hasattr(
                            m, 'conv2_offset'):
                        constant_init(m.conv2_offset, 0)

            if self.zero_init_residual:
                for m in self.modules():
                    if isinstance(m, BottleneckV1s):
                        constant_init(m.norm3, 0)
                    elif isinstance(m, BasicBlockV1s):
    ...
    def forward(self, x):
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            if i in self.out_indices:
                outs.append(x)
        if self.jpu:
            return tuple(self.jpu(outs))
        else:
            return tuple(outs)

    def train(self, mode=True):
        super(ResNetV1s, self).train(mode)
        self._freeze_stages()
