...
def merge_aug_segmaps(results,
                      num_classes,
                      mode='average',
                      aug_weights=None,
                      return_proba=False):
    if aug_weights:
        assert isinstance(aug_weights, (list, tuple)) and \
            len(aug_weights) == len(results)
    assert mode in ['average', 'vote']
    if mode == 'average':
        if aug_weights is not None:
            merge_results = np.average(
                np.array(results), axis=0, weights=np.array(aug_weights))
        else:
