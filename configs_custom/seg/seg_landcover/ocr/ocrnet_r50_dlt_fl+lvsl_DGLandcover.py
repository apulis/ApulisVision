...
model = dict(
    ...
    mask_head=dict(
        ...
        in_channel=2048,
        in_channel_aux=1024,
        inter_channel_aux=512,
        drop_out=0.05,
        num_classes=7,
        upsample_cfg=dict(type='bilinear', upsample_size=input_size),
        loss_mask=dict(
            type='FocalLoss', gamma=0.3, loss_weight=0.527),
        loss_mask_aux=dict(
            type='LovaszSoftmax', loss_weight=0.263),
        loss_context=dict(
            type='CrossEntropyLoss', loss_weight=0.21)))
...
log_level = 'INFO'
work_dirs = './work_dirs/ocrnet_r50_dlt_fl+lvsl_DGLandcover'
label_suffix = '_mask'
load_from = None
resume_from = None
